...
const Trayectoria: FC<TrayectoriaProps> = ({ slice }) => {
  return (
    <section
      ...
      className="scroll-mt-24 bg-white py-16 transition-colors duration-300 dark:bg-night-dark md:py-24"
    >
            ...
            <ol className="relative flex flex-col gap-7 border-l-2 border-accent/40 pl-7 md:pl-10">
              {slice.primary.milestones.map((milestone, index) => (
                <li key={`${milestone.year}-${index}`} className="relative">
                  <span className="absolute left-[-2.1rem] top-1 h-4 w-4 rounded-full border-4 border-white bg-accent dark:border-night-dark md:left-[-2.65rem]" />
                  {milestone.year && (
